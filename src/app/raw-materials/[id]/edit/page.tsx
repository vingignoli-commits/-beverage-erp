...
export default function EditRawMaterialPage() {
    ...
    async function loadData() {
      try {
        const [materialRes, suppliersRes] = await Promise.all([
          fetch(`/api/raw-materials/${id}`, { cache: "no-store" }),
        ...
        if (!materialRes.ok) {
          throw new Error(materialJson.message || "No se pudo cargar el insumo.");
        }

        if (!suppliersRes.ok) {
          throw new Error(
            suppliersJson.message || "No se pudieron cargar los proveedores."
          );
        }
        ...
        setForm({
          ...
          unit: material.unit ?? "",
          costPerUnit: String(material.costPerUnit ?? ""),
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || "No se pudo actualizar el insumo.");
      }
    ...
    <main style={mainStyle}>
      <div style={containerStyle}>
        <div style={headerStyle}>
          <p style={eyebrowStyle}>Editar insumo</p>
          <h1 style={titleStyle}>Modificar insumo</h1>
        </div>
